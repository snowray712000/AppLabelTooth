...
vertCollepse1.set_is_open(True)
...
def reload_and_update_listboxview():
    ...
    def find_index():
        if AppGlobals.upper_lower == "upper":
            return next((i for i, x in enumerate(path_idds_upper) if x[1] == AppGlobals.idd and x[2] == AppGlobals.ver), None)
        else:
            return next((i for i, x in enumerate(path_idds_lower) if x[1] == AppGlobals.idd and x[2] == AppGlobals.ver), None)
            
    list1.selected_index = find_index()
